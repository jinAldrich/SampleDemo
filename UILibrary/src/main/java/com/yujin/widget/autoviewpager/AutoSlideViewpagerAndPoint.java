package com.yujin.widget.autoviewpager;
 ...
 *          3：通过构造函数传入上下文与imageViews AutoSlideViewpagerAndPoint vp=new
 *          AutoSlideViewpagerAndPoint(MainActivity.this, imageViews);
 *          4:调用方法生成整体的layout FrameLayout ly=vp.setViewPagerAndPoint();
 *          5:将生成的layout加入到你要用的布局中 li.addView(ly);
 * 
 */
public class AutoSlideViewpagerAndPoint {
	...
	public static class Builder {
		...
		public FrameLayout build() {
			AutoSlideViewpagerAndPoint vp = new AutoSlideViewpagerAndPoint(context, imageViews);
	...
	public AutoSlideViewpagerAndPoint(Context context, ArrayList<ImageView> imageViews) {
		this.imageViews = imageViews;
		this.context = context;
	}
	...
	private LinearLayout setDots(Zhang_ViewPager viewPager) {
		...
		for (int i = 0; i < imageViews.size(); i++) {
			...
			LayoutParams fl = new LayoutParams(dotHeight, dotHeight);
			fl.leftMargin = 2;
			fl.rightMargin = 2;
